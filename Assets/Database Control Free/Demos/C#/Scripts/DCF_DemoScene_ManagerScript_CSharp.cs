...
public class DCF_DemoScene_ManagerScript_CSharp : MonoBehaviour {
    ...
    {
        IEnumerator e = DCF.RegisterUser(playerUsername, playerPassword, "Donnée par défaut"); // << Send request to register a new user, providing submitted username and password. It also provides an initial value for the data string on the account, which is "Hello World".
        while (e.MoveNext())
        {
            yield return e.Current;
        }
